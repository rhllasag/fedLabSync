...
from fedLabSync.evaluation.metrics import class_wise_kappa
from fedLabSync.utils.system import find_and_set_gpus
from fedLabSync.utils.scriptutils import (assert_project_folder,
                                     get_splits_from_all_datasets,
                                     add_logging_file_handler,
                                     with_logging_level_wrapper)
from fedLabSync.evaluation.dataframe import (get_eval_df, add_to_eval_df,
...
def get_and_load_model(project_dir, hparams, weights_file_name=None, clear_previous=True):
    ...
    if not weights_file_name:
        from fedLabSync.models.model_init import init_and_load_best_model
        ...
        from fedLabSync.models.model_init import init_and_load_model
...
def plot_hypnogram(out_dir, pred, id_, true=None):
    ...
    """
    from fedLabSync.evaluation.plotting import plot_and_save_hypnogram
...
def plot_cm(out_dir, pred, true, n_classes, id_):
    ...
    """
    from fedLabSync.evaluation.plotting import plot_and_save_cm
...
def _predict_sequence(study_pair, seq, model, verbose=True):
    ...
    """
    from fedLabSync.utils.scriptutils.predict import sequence_predict_generator
...
def predict_on(study_pair, seq, model=None, model_func=None, n_aug=None,
    ...
    if not seq.margin:
        ...
        from fedLabSync.utils.scriptutils.predict import predict_on_generator
        if n_aug:
...
def get_sequencer(dataset, hparams):
    ...
    from fedLabSync.sequences import get_batch_sequence
...
def run_pred_and_eval(dataset,
    ...
    for i, sleep_study_pair in enumerate(dataset):
        ...
        if args.wake_trim_min:
            ...
            from fedLabSync.bin.cm import wake_trim
